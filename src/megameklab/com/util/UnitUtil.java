...
public class UnitUtil {
    ...
    /*
     * A handheld weapon must include enough heat sinks for the maximum heat
     * generated by all energy weapons.
     * 
     */
    public static int getHandheldHeat(HandheldWeapon hhw) {
        int heat = 0;
        for (Mounted m : hhw.getWeaponList()) {
            WeaponType wt = (WeaponType) m.getType();
            if (((WeaponType)m.getType()).hasFlag(WeaponType.F_ENERGY)) {
                heat += wt.getHeat();
            }
        }
        for (Mounted m : hhw.getMisc()) {
            ...
            if (mtype.hasFlag(MiscType.F_LASER_INSULATOR) && heat > 0) {
                heat--;
            }
        }
        return heat;    	
